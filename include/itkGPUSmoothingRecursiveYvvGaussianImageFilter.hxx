...
namespace itk
{
    ...
    ::SetUp(ScalarRealType spacing)
    {
        ...
        if(this->GetSigma()>=0.5)
        {
            ...
            m_CPUMatrix[7] = (m_B1 * m_B2 + m_B3 * m_B2 * m_B2 - m_B1 * m_B3 * m_B3 - m_B3 * m_B3 * m_B3 - m_B3 * m_B2 + m_B3)/factor;
    ...
    ::SetSigmaArray( const SigmaArrayType & sigma )
    {
        ...
        if (this->m_Sigma != sigma)
        {
            ...
            {
                #ifdef VERBOSE
                std::cout<< telltale << ". GPUSmoothing::SetSigmaArray. pixelSize: ["<<pixelSize[0]<<","<<pixelSize[1]<<","<<pixelSize[2]<<"] \n";
                #endif
...
/*#ifdef VERBOSE
    ...
    ::GenerateInputRequestedRegion() throw(InvalidRequestedRegionError)
    {
        ...
        typename GPUSmoothingRecursiveYvvGaussianImageFilter<TInputImage,TOutputImage>::InputImagePointer image = const_cast<InputImageType *>( this->GetInput() );
        if( image )
        {
